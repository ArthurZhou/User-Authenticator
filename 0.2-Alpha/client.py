...
"""
User-Authenticator Alpha0.2 Client
By ArthurZhou
...
def signup():
    ...
    def wriUP():
        ...
        if str(askyn) == 'True':
            ...
            time.sleep(0.1)
            s.send(usr.get().encode('utf-8'))    # send username
            time.sleep(0.1)
...
def check(login, nm, psw, s):
    ...
    s.send(nm.get().encode('utf-8'))    # send salt to the server
    time.sleep(0.1)
...
def main():
    ...
    bt = tk.Button(login, text='Login', font=('', 15), command=lambda: check(login, nm, psw, s))   # show login button
